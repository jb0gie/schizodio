<script lang="ts">
  ...
  import AdoptionToast from '../components/AdoptionToast.svelte';
  ...
  import { Contract, Account, RpcProvider, Call } from 'starknet';
  ...
  // Adoption toast state
  let showAdoptionToast = false;
  let adoptionToastData = {
    tokenId: 0,
    walletAddress: '',
    price: '',
    token: '',
  };
  let adoptionToastLoop = false;
  let adoptionToastLoopInterval: ReturnType<typeof setInterval> | null = null;
  ...
  onDestroy(() => {
    ...
    // Cleanup adoption toast loop interval
    if (adoptionToastLoopInterval) {
      clearInterval(adoptionToastLoopInterval);
    }
  });
  ...
  async function mintNFT() {
    ...
    try {
      ...
      // Show adoption toast
      adoptionToastData = {
        tokenId: lastMintedId,
        walletAddress: walletAddress,
        price: selectedPaymentToken.price,
        token: selectedToken,
      };
      showAdoptionToast = true;

      // Random celebration popup (existing)
      setTimeout(() => {
  ...
  // Function to test adoption toast (for debugging)
  function testAdoptionToast() {
    console.log('Testing adoption toast...');

    // Use current adoption data if available, otherwise use test data
    const useCurrentAdoption = lastMintedId !== null;

    adoptionToastData = {
      tokenId: useCurrentAdoption ? lastMintedId! : 42,
      walletAddress:
        useCurrentAdoption && walletAddress
          ? walletAddress
          : '0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef',
      price:
        useCurrentAdoption && selectedPaymentToken
          ? selectedPaymentToken.price
          : '200',
      token: useCurrentAdoption ? selectedToken : 'STRK',
    };

    showAdoptionToast = true;

    if (useCurrentAdoption) {
      console.log(`Using current adoption data - Token ID: ${lastMintedId}`);
    } else {
      console.log('Using test data - no current adoption available');
    }
  }

  // Function to toggle adoption toast loop
  function toggleAdoptionToastLoop() {
    adoptionToastLoop = !adoptionToastLoop;

    if (adoptionToastLoop) {
      console.log('Starting adoption toast loop...');
      // Start the loop - show toast every 10 seconds
      adoptionToastLoopInterval = setInterval(() => {
        testAdoptionToast();
      }, 10000);
      ...
      console.log('Stopping adoption toast loop...');
      if (adoptionToastLoopInterval) {
        clearInterval(adoptionToastLoopInterval);
        adoptionToastLoopInterval = null;
      }
      showAdoptionToast = false;
    }
  }
</script>
...
<!-- Adoption Toast -->
<AdoptionToast
  tokenId={adoptionToastData.tokenId}
  walletAddress={adoptionToastData.walletAddress}
  price={adoptionToastData.price}
  token={adoptionToastData.token}
  visible={showAdoptionToast}
  onClose={() => {
    showAdoptionToast = false;
  }}
/>
  ...
  <div class="w-full max-w-screen-xl mx-auto px-4">
    <Window title="📢 SCHIZO NEWS NETWORK 📢" width="full">
      ...
      <div class="p-2 text-center">
        <div class="text-xs flex gap-2 justify-center flex-wrap">
          <Button on:click={testAudio}>CLICK HERE</Button>
          <Button on:click={testAdoptionToast}>🎭 TEST ADOPTION TOAST</Button>
          <Button on:click={toggleAdoptionToastLoop}>
            {adoptionToastLoop ? '⏹️ STOP LOOP' : '🔄 LOOP TOAST'}
          </Button>
